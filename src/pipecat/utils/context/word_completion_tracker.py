...
class WordCompletionTracker:
    ...
    def _force_complete(self, word: str) -> bool:
        ...
        if self._llm_text is not None:
            ...
            self._llm_consumed = self.get_remaining_llm_text()
    ...
    def get_remaining_llm_text(self) -> str | None:
        ...
        if self._llm_text is None:
            return None
        # Dubit Edit: a suppressed word can leave pending context before the spoken cursor.
        remaining = self._llm_text[min(self._llm_pos, self._llm_spoken_pos) :].strip()
